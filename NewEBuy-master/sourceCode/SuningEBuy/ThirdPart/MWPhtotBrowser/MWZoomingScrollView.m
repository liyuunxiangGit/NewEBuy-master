...
- (void)displayImage {
	if (_photo && _photoImageView.image == nil) {
		...
		if (img) {
			...
			photoImageViewFrame.size = CGSizeMake(img.size.width, img.size.height);
			_photoImageView.frame = photoImageViewFrame;
			self.contentSize = photoImageViewFrame.size;
            if(_commentDelegate && [_commentDelegate respondsToSelector:@selector(didshowImage:)])
            {
                [_commentDelegate performSelector:@selector(didshowImage:) withObject:_commentDelegate];
            }
			// Set zoom to minimum zoom
			[self setMaxMinZoomScalesForCurrentBounds];
			//NSLog(@"photoImageViewFrame =%@",NSStringFromCGRect(_photoImageView.frame));
		} else {
...
- (void)setMaxMinZoomScalesForCurrentBounds {
  ...
  //  NSLog(@"_photoImageView =%@", NSStringFromCGRect(_photoImageView.frame));
	[self setNeedsLayout];
