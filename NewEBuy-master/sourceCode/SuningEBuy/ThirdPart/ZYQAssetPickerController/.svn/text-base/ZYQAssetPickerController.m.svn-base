    ...
    if (self.contentView.subviews.count<assets.count) {
        for (int i=0; i<assets.count; i++) {
            if (i>((NSInteger)self.contentView.subviews.count-1)) {
                ...
                [assetView bind:assets[i] selectionFilter:selectionFilter isSeleced:[_delegate didSelectedAsset:assets[i]]];
                ...
                [(ZYQAssetView*)self.contentView.subviews[i] bind:assets[i] selectionFilter:selectionFilter isSeleced:[_delegate didSelectedAsset:assets[i]]];
            }
        ...
        for (int i=self.contentView.subviews.count; i>0; i--) {
            if (i>assets.count) {
                ...
                [(ZYQAssetView*)self.contentView.subviews[i-1] bind:assets[i-1] selectionFilter:selectionFilter isSeleced:[_delegate didSelectedAsset:assets[i]]];
            }
        }
    }
}
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell bind:tempAssets selectionFilter:picker.selectionFilter minimumInteritemSpacing:minimumInteritemSpacing minimumLineSpacing:minimumLineSpacing columns:columns assetViewX:1];
...
-(ALAsset*)selectedAssetByAsset:(ALAsset*)asset{
    for (ALAsset *item in _indexPathsForSelectedItems) {
        if([[[item defaultRepresentation].url absoluteString] isEqualToString:[[asset defaultRepresentation].url absoluteString]]) {
            return item;
        }
    }
    return nil;
...
-(BOOL)didSelectedAsset:(ALAsset *)asset
{
    ALAsset *selectedAsset = [self selectedAssetByAsset:asset];
    if (selectedAsset) {
        return YES;
    }return NO;
}
...
- (void)didDeselectAsset:(ALAsset *)asset
{
    ALAsset *selectedAsset = [self selectedAssetByAsset:asset];
    if (selectedAsset) {
        [_indexPathsForSelectedItems removeObject:selectedAsset];
    }else{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    vc.indexPathsForSelectedItems = [NSMutableArray arrayWithArray:picker.selectAssets];
