...
- (void)dealloc {
    ...
    TT_RELEASE_SAFELY(_visiblePages);
    TT_RELEASE_SAFELY(_reusablePages);
    TT_RELEASE_SAFELY(_deletedPages);
...
- (void)setNumberOfPages:(NSInteger)number
{
    ...
    if (_visibleIndexes.location+_visibleIndexes.length > _numberOfPages)
    {
        ...
        maxLength = _numberOfPages > maxLength ? maxLength : _numberOfPages;
        _visibleIndexes.location = _numberOfPages - maxLength;
        _visibleIndexes.length = maxLength;
    }
...
- (void)updateVisiblePages
{
	...
	else if(leftViewOriginX < -pageWidth && [_visiblePages count] > 0)
    {
	...
	if (rightViewOriginX > _maxWidth && [_visiblePages count] > 0) {
