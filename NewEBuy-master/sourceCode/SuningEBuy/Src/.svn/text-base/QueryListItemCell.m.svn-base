...
#define MOBILE_QUERY_CELL_HEIGHT 142

@implementation QueryListItemCell
...
- (void)setItemDTO:(MobileQueryDTO *)itemDTO
{
    if (itemDTO != _itemDTO)
    {
        TT_RELEASE_SAFELY(_itemDTO);
        ...
        NSString *desc = [[_itemDTO.orderName substringFromIndex:12] stringByReplacingOccurrencesOfString:@"【" withString:@""];
        NSString *name = [desc stringByReplacingOccurrencesOfString:@"】" withString:@""];
        
        self.orderNameTextLabel.text = IsStrEmpty(name)?@"--":name;
        
        self.companyNameTextLabel.text = IsStrEmpty(_itemDTO.orderName)?@"--":[NSString stringWithFormat:@"%@：%@",L(@"Phone number"),[_itemDTO.orderName substringToIndex:11] ];
        
        self.payAmountLabel.text = L(@"VPTotal");
        self.payAmountTextLabel.text = [[NSString stringWithFormat:@"￥ %.2f", [_itemDTO.salePrice floatValue]] stringByAppendingString:L(@" yuan")];
        
        self.payTimeTextLabel.text = [_itemDTO.creatTime substringToIndex:10];
        
        self.statusNameTextLabel.text = _itemDTO.statusDesc;
        
        if ([name rangeOfString:L(@"Telecom")].length > 0)
        {
            self.icon.image =[UIImage imageNamed:@"Telecom_rgb.png"];
        }
        else if ([name rangeOfString:L(@"Mobile")].length > 0)
        {
            self.icon.image =[UIImage imageNamed:@"Mobile_rgb.png"];
        }
        else if ([name rangeOfString:L(@"Unicom")].length > 0)
        {
            self.icon.image =[UIImage imageNamed:@"Unicom_rgb.png"];
        }
        else
        {
            self.icon.image =[UIImage imageNamed:@"order_MobileIcon.png"];
        }
...
+ (CGFloat)height:(MobileQueryDTO *)item
{
    if (!item)
    {
...
- (void)layoutSubviews
{
    ...
    self.payTimeTextLabel.frame =CGRectMake(205, 10, 100, 20);
