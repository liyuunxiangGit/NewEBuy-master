...
@protocol LotteryListViewControllerDelegate;



@interface LotteryListViewController : CommonViewController<LotteryListFootViewDelegate,LotteryProtocolCellDelegate,UINavigationBarDelegate,CouponServiceDelegate>{
    
    id<LotteryListViewControllerDelegate>   __weak delegate;
...
@property (nonatomic,assign) int                 periods; //倍数
@property(nonatomic,weak) id<LotteryListViewControllerDelegate>delegate;
...
@protocol LotteryListViewControllerDelegate <NSObject>
@optional
//编辑一行
...
//删除一行
-(void)returnUserDeleteLotteryCell:(NSMutableArray *)array andMultiNo:(int)multiNo
                           periods:(int)periods andOnlyCouldChooseMoney:(CGFloat)money;
//手选一注
-(void)returnUserAddLotteryCell:(NSMutableArray *)array andMultiNo:(int)multiNo
                        periods:(int)periods  andOnlyCouldChooseMoney:(CGFloat)money;
//刷新选球页面的倍数
