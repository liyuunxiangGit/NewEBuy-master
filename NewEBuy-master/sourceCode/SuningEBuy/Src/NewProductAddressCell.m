...
#import "NewProductAddressCell.h"

#import "NSAttributedString+Attributes.h"

@interface NewProductAddressCell()
{
    BOOL        isRushProduct;
}
...
@implementation NewProductAddressCell
...
- (void)setItem:(DataProductBasic *)productDetail qiang:(PanicPurchaseDTO*)dto{
    ...
    //    //是否为C店商品，1是，0不是
    //    if(self.productDetailDTO.isCShop == 1)
    //    {
    ...
    //        else
    //        {
    //            arriveTime = @"很抱歉,此商品库存不足";
    //        }
    //
    //    }
    //    else if(self.productDetailDTO.isCShop == 0)
    //    {
    if([self.productDetailDTO.suningPrice doubleValue] > 0)
    {
...
- (void)setItem:(DataProductBasic *)productDetail
{
    ...
    if(self.productDetailDTO.isCShop == 1)
    {
        ...
        {
            arriveTime = L(@"Sorry, Not enough stock");//@"很抱歉,此商品库存不足";
