...
private:
  ZXING_Decoder* decoder;
public:
ZXingWidgetControllerCallback(ZXING_Decoder* _decoder) : decoder(_decoder) {}
...
@interface ZXING_Decoder ()
@property(nonatomic, retain) UIImage *subsetImage;
@end

@implementation ZXING_Decoder
...
- (ArrayRef<char>) prepareSubset {
  ...
  UIImage *tempImg = [[UIImage alloc] initWithCGImage:subsetImageRef];
  self.subsetImage = tempImg;
  [tempImg release];
    ...
    for (int i = 0; !decoderResult && i < 4; i++) {
      ...
      for (FormatReader *reader in formatReaders) {
        ...
        try {
          ...
          decoderResult.formatReader = NSStringFromClass([reader class]);
